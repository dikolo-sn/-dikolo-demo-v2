...
F_VENTES = "data/ventes.json"
F_ACHATS = "data/achats.json"
F_USERS = "data/users.json"
F_ROLES = "data/roles.json"
F_IPM = "data/ipm.json"
F_PAIEMENT = "data/paiements.json"
F_RETOURS = "data/retours.json"
F_INVENTAIRE = "data/inventaire.json"
F_DEPENSES = "data/depenses.json"
F_BLS = 'data/bls.json'
F_LOTS = "data/lots.json"
F_RAYONS = "data/rayons.json"
F_ETAGERES = "data/etageres.json"
F_PANIER = "data/panier.json"
DATA_DIR = "data"
...
def lire(fichier):
    if not os.path.exists(fichier) or os.path.getsize(fichier) == 0:
        return {} if "users" in fichier or "roles" in fichier else []
...
for f in [F_PRODUITS, F_VENTES, F_ACHATS, F_USERS, F_ROLES, F_IPM, F_PAIEMENT, F_RETOURS, F_INVENTAIRE, F_DEPENSES, F_LOTS]:
    if not os.path.exists(f):
        if f == F_USERS:
            sauver(f, {"admin": {"username": "admin", "password": "1234", "role": "admin"}})
        elif f == F_ROLES:
            sauver(f, {"admin": list(PERMISSIONS_LISTE.keys()), "caissier": ["voir_dashboard", "voir_caisse", "voir_historique"], "commercial": ["voir_dashboard", "voir_produits", "voir_achat", "voir_vente", "voir_historique"]})
        elif f == F_IPM:
            sauver(f, [{"nom": "Client Anonyme", "taux": 0}, {"nom": "CNAM 80%", "taux": 80}])
        elif f == F_PAIEMENT:
            sauver(f, [{"nom": "Cash"}, {"nom": "Orange Money"}, {"nom": "Wave"}])
        else:
            sauver(f, [])
...
def build_menu(current_user, active_page="dashboard"):
    menu = "<div class='nav'><div class='nav-top'><span class='logo-text'>DiKoLo (Digante-Koungheul-Lour)</span></div>"
    menu += "<div class='nav-links'>"
    menu += f"<a href='/dashboard' class='{'active' if active_page=='dashboard' else ''}'>Accueil</a>"
    menu += "<a href='/caisse'>Caisse</a><a href='/etatcaisse'>Etat Caisse</a><a href='/facturation'>Facturation</a>"
    menu += "<a href='/reception'>Commandes/Bls</a><a href='/produits'>Produits</a><a href='/vendre'>Vente</a>"
    menu += "<a href='/historique'>Historique</a><a href='/alertes'>Alertes</a><a href='/stats'>Statistiques</a>"
    menu += "<a href='/inventaire'>Inventaire</a><a href='/export'>Exportations</a><a href='/users'>Utilisateurs</a>"
    menu += "<a href='/logout' class='logout-btn'>Déconnexion</a></div></div>"
    return menu
...
.login-box{background:white; padding:40px; border-radius:15px; width:350px; margin:80px auto; box-shadow:0 5px 20px rgba(0,0,0,0.2); text-align:center}
.error{color:#e74c3c; padding:10px; background:#fadbd8; border-radius:5px; margin-bottom:10px}
</style>"""
...
async def login_get(request: Request, msg: str = ""):
    ...
    {CSS_GLOBAL}
    <div class='login-box'>
        <h1>DiKoLo</h1><h3>Gestion + Caisse</h3>
        {err}
        <form method='post'>
...
async def login_post(request: Request, username: str = Form(...), password: str = Form(...)):
    ...
    for token, user in users.items():
        if user["username"] == username and user["password"] == password:
            ...
            return response
    return RedirectResponse(url="/login?msg=Identifiants incorrects", status_code=303)

@app.get("/dashboard")
async def dashboard(current_user: dict = Depends(require_permission("voir_dashboard"))):
    if isinstance(current_user, RedirectResponse): return current_user
    menu = build_menu(current_user, "dashboard")
    html = f"{CSS_GLOBAL}<div class='container'>{menu}<h1>Bienvenue {current_user['username']}</h1></div>"
    return HTMLResponse(html)
...
# Ajoute ici toutes tes autres routes: /produits, /vendre, /caisse etc
@app.get("/alertes")
